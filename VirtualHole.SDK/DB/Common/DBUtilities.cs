...
namespace VirtualHole.DB
{
	internal static class DBUtilities
	{
		...
		public const string TField = "_t";
		public const string LastOperationTimestampFieldName = "_lastOperationTimestamp";
			...
			IEnumerable<T> objs, DateTime timestamp, CancellationToken cancellationToken = default)
		{
			...
			BsonDocument NotEqualTimestampFilter()
			{
				return new BsonDocument(
					LastOperationTimestampFieldName,
					new BsonDocument(
						"$not", 
		...
		public static BsonDocument ToBsonDocumentWithTimestamp<T>(T obj, DateTime timestamp)
		{
			...
			objBSON.Add(new BsonElement(LastOperationTimestampFieldName, timestamp));
			return objBSON;
		}
	}
}
